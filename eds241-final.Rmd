...
The penalty for houses being located near the incinerator is `r -1 * round(model1$coefficients[2], 3)`. This coefficient does correspond to the causal effect of the incinerator because it is demonstrating that house values will significantly decrease if a house a near an incinerator compared to a house that is not. There is a possibility that omitted variables are exaggerating this causal relationship. 
...
```{r}
# DD_cap1 <- lm_robust(formula = rprice ~ nearinc, data=data)
# summary(DD_cap1)
...
# these two should produce the same thing if we added year in the lm_robust one above. So we should discuss which one is correct cause should we be controlling for year, I assume so. 

```

The estimated DD coefficient is `r DD1$coefficients[[1]]`. This means that for houses that are near the incinerator, their value decreases (negative sign) by `r -1* DD1$coefficients[[1]]` (magnitude).
...
```{r}
DD2 <- plm(rprice ~ nearinc + age + rooms + area + land,
               index = c("year"), 
               model = "within", 
               effect = "twoways", 
               data = data)
